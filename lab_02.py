...
def calculate_similarities(v1_bin, v2_bin, v1_full, v2_full):
    # Jaccard & SMC (Binary attributes only)
    ...
    # Cosine Similarity (Complete numeric vectors)
...
def task_optional_o1(file_path):
    # Extracts a square matrix (3x3) and repeats A1 logic
    df = pd.read_excel(file_path, sheet_name="Purchase data").dropna(how='all').dropna(axis=1, how='all')
...
def main():
    ...
    # plot_scatter(a3_results[9]) # Uncomment to view plot
    
    print("=== A4-A9: THYROID DATA ===")
    df_thy = clean_thyroid_data(file)
    
    # Binary extraction for A5
    ...
    # Numeric encoding for A6
    df_enc = df_thy.copy()
    for c in binary_cols: df_enc[c] = df_enc[c].map({'t': 1, 'f': 0})
    df_enc.fillna(0, inplace=True)
    v1_full = pd.to_numeric(df_enc.iloc[0].drop(['Record ID', 'referral source', 'Condition', 'sex'], errors='ignore')).values
    v2_full = pd.to_numeric(df_enc.iloc[1].drop(['Record ID', 'referral source', 'Condition', 'sex'], errors='ignore')).values
    ...
    # task_a7_heatmap(df_enc, n=20) # Uncomment to view heatmap
